...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (tabs.length === 0) return; // Only run if auth tabs exist
...
function switchTab(targetId) {
    const tab = document.querySelector(`.auth_tab[data-target="${targetId}"]`);
    if (tab) {
        tab.click();
    }
}
...
function selectHotel(hotelId) {
    ...
    document.getElementById('summary-hotel-price').textContent = `₹${hotel.price.toLocaleString()}/Night`;
...
function showConfirmation() {
    ...
    document.getElementById('confirm-price').textContent = `₹${totalCost.toLocaleString()}`;
